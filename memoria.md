...
La finalidad del proyecto es añadir una capa de seguridad a una red wifi (en este caso la red del colegio IES ROMERO VARGAS) haciéndola más segura con un servidor Radius. Además, como un servidor Radius no es algo sencillo de manejar, se proporcionará una interfaz a través de una pagina web para facilitar su control.
...
En este apartado se procede explicar el proyecto. Cómo ha ido progresando a lo largo del tiempo y cómo ha evolucionado con el tiempo. Para facilitar la comprensión,  se incluirán capturas e imágenes.
...
El funcionamiento se basa en que donde antes la gestión de la autenticación de un proceso la gestionaba un servicio, como la contraseña de un router, ahora la va a llevar a cabo un servidor Radius. Cuando un usuario quiera autenticarse, dicho servicio o aplicación, manda la petición al Radius. Radius la evalúa y le indica al router si ese usuario es válido o no.
...
Se va a proceder a hacer muchas maniobras con la base de datos de Freeradius. Por lo que es necesario ver de cerca la base de datos del servidor freeradius, para que más tarde se pueda entender todos los procesos y pasos realizados.
...
- **`radreply`:** Un usuario puede tener varios campos, aparte de su contraseña, como la mac del dispositivo al cual se conecta, cuantas veces puede conectarse de forma simultánea... En **`radcheck`** sólo debería estar el usuario junto a su contraseña. En esta tabla se puede introducir campos para los usuarios.
- **`radgroupcheck`:** Radius puede gestionar grupos con usuarios, ejecutando una condición a todos los usuarios del grupo por igual.
...
Debe quedar claro que no es específicamente necesario el usar un mikrotik para el ejecutar este proyecto. Cualquier router que permita usar como medio de autenticación wifi una  `WPA-Enterprise` es un router válido. 

Yo personalmente no tengo ningún mikrotik en mi domicilio, tengo un router doméstico que me proporciona mi ISP (Orange) en mi caso un modelo livebox, para la fibra óptica. He intentado hacer funcionar el radius con mi router, pero no contiene `WPA-Enterprise` y con la pequeña investigación que he hecho, Orange capa esa función en todos sus routers que no sean para empresas. Con esto quiero decir que muy probablemente vuestro router no sea un router válido para configurarlo junto a un servidor radius, a menos que os hayáis comprado un router propio.

Como he comentado antes no tengo ningún mikrotik así que, intente hacer las pruebas con una maquina virtual. Me creé una cuenta en la pagina web de mikrotik, me descargué una iso y una licencia de prueba. Monté la máquina virtual con Vmware  y configuré el mikrotik como un ap-bridge. Funcionaba correctamente e incluso el servidor radius, daba el aviso de que el router se había conectado, en principio parecía que todo estaba bien. Pero aunque la consola de Freeradius, indicará que ha había habido conexión entre el router y el servidor, los usuarios creados no se autenticaban. 

La maquina virtual tenía una tarjeta de red wifi y podía conectarse a un punto de acceso, pero aunque la interfaz sea wifi, dentro de las interfaces de la maquina virtual, solo aparecían interfaces ethernet. Si sólo aparecen interfaces ethernet, no es posible hacer una conexión wifi y sin ella no es posible autenticar mediante radius para hacer las pruebas.

El problema reside en que la maquina virtual crea un puente entre la maquina real y la virtual para todos los interfaces conectados. Aunque esos interfaces sean wifi, al fin y al cabo el bridge que se crea es tratado como un interfaz ethernet. Esto no dificulta la conexión hacia el exterior por parte del router, pero si lo hace para crear un punto de acceso y habilitar la conexión de fuera hacia adentro, no es posible hacerlo.

Debido a que mi router no era válido para hacer pruebas y no puede ejecutar virtualmente un router mikrotik para las pruebas, tuve que solicitar un router mikrotik al colegio. Y es con el que se va a realizar todo el proyecto.

Tuve problemas también a la hora de configurar el **`mikrotik`** como ap-bridge, ya que a veces fallaba a la hora de asignar ips. Era un simple problema de configuración. Mas tarde me di cuenta de que existe un apartado para configurar el mikrotik según unos perfiles y entre ellos había uno de ap-bridge.
...
Principalmente, tenemos un router que es capaz de logar usuarios a través de un servidor radius, y dicho servidor es capaz de gestionar usuarios, grupos y routers a través de una base de datos MySQL. Además también ofrece información de dichos usuarios.
...
En un primer momento creé una pagina web, que simplemente mostraba los grupos, sus usuarios y los routers, los insertaba y los borraba.

Después, leyendo el manual de php, tuve problemas al insertar los datos desde un archivo csv, Una vez comprendes que ese archivo sólo se tratan de filas compuestas de columnas (como una tabla de una base de datos), es mucho más fácil de manejar.

Una vez que llegué a completar el punto comentado anteriormente, se plantea el siguiente problema:

Echar usuarios de la red durante un periodo de tiempo.

 Radius es capaz de bloquear usuarios, añadiendo un usuario y un campo con un valor `reject` dentro de su propia base de datos, pero puede dar errores, porque al no haber prioridad de ordenes como en los grupos, los datos en las tablas pueden contradecirse y dar un resultado distinto en cada autenticación.
...
La forma en la que ambas bases de datos se actualizan mediante cron, que se ejecuta un script php en el menor tiempo posible.

### Base de datos 

La base de datos que voy a mostrar ahora es la versión final de la base de datos creada por mí, como base de datos secundaría donde se va guardar la palabra. Voy a expresar todo el proceso de creación y porque cada cosa es qué. 

![](img_memoria/sincrono.png)

Esta base de datos tiene tablas como usuario, cliente , grupo y usuario grupo que guardan la información de los router, usuarios... Alguna de esta informacíon es posible insertarla en la base de datos de radius, pero es necesario almacenar toda esa información ya que va a ser borrada de forma peródica por los profesores.

Ahora con ambas bases de datos, si queremos que un usuario no tenga acceso, simplemente lo eliminamos de la base de datos de radius. Si queremos que vuelva a funcionar lo insertamos.

Todo va ir girando con sacar datos de la base de datos de radius o meterlo para que funcione o no.

De la misma forma en la base de datos de radius, existía el concepto de grupo. Aquí también está pero la diferencia es que los grupos no se pasan de una base de datos a otra, si no que solo se pasan los usuarios. Si queremos eliminar un grupo entero, simplemente eliminamos todos los usuarios de ese grupo. 
...
Al igual que con los usuarios, los routers/clientes que usan el servidor radius (ya que pueden ser tantos como  se quiera), pueden ser añadidos también mediante csv.´´´´´´´´´´´´´´´´´´´´´´´´´´´´´´
...
Aquí se puede ver un ejemplo, de la lista de los routers, su contraseña y una descripción. He de señalar que la contraseña del router no está encriptada, no por el hecho de que no se puede ver en la lista, porque eso es lo de menos, si no porque si se hace una encriptación por md5 u otro tipo de funciones para encriptar, tendríamos que poner el hash en el router, para que EAP lo tunelice, y lo compare con nuestro hash (EAP no se pensó para que las contraseñas viajaran ya encriptadas, si no en texto plano), por lo que tener un hash en vez de tener una contraseña en texto plano es lo mismo.
